...
pub fn solve() -> () {
  let line : Vec<i64> = io::stdin()
      ...
      .next()
      .unwrap()
      .map(|line| line.chars().map(|c| c.to_string().parse::<i64>().unwrap()).collect::<Vec<i64>>()).unwrap();
  ...
  let checksum: i64 = file.iter().enumerate().map(|(index, file)| index as i64 * file.unwrap()).sum();
