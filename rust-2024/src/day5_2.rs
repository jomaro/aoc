...
fn rec(index: i64, graph: &HashMap<i64, Vec<i64>>, visited : &mut HashSet<i64>, stack: &mut Vec<i64>) {
...
fn is_ordered(update: &&Vec<i64>, rules: &Vec<(i64, i64)>) -> bool{
  let order : HashMap<i64, i64> = update.into_iter().enumerate().map(|(index, value)| (value.clone(), index as i64)).collect();

  rules.iter().all(|(n1, n2)| {
    match ( order.get(&n1), order.get(&n2) ) {
      (None, _) => true,
      (_, None) => true,
...
fn parse() -> (Vec<(i64, i64)>, Vec<Vec<i64>>) {
  ...
  for line in io::stdin().lock().lines() {
    ...
    if ! finished_rules {
      let rule: Vec<i64> = line.split("|").map(|number| number.parse::<i64>().unwrap()).collect();

      rules.push((rule[0], rule[1]));
    } else {
      let update: Vec<i64> = line.split(",").map(|number| number.parse::<i64>().unwrap()).collect();
