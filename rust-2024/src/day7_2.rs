...
fn rec(numbers: &Vec<i64>, index: i64, size: i64, acc: i64, result: i64) -> bool {
  ...
  return rec(numbers, index +1, size, acc + numbers[index as usize], result) || 
    rec(numbers, index +1, size, acc * numbers[index as usize], result) ||
    rec(numbers, index +1, size, my_concat(acc, numbers[index as usize]), result);
}
